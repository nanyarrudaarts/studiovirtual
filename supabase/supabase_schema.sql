-- ==============================================================================
-- STUDIO VIRTUAL — SUPABASE DATABASE SCHEMA & RLS MIGRATION
-- ==============================================================================
...
CREATE TRIGGER set_updated_at_artista
    BEFORE UPDATE ON public.artista
...
-- ─── 7. TABELAS DE JUNÇÃO (MANY-TO-MANY) ──────────────────────────────────────
...
CREATE POLICY "Users can manage own artist profile" ON public.artista
...
CREATE POLICY "Users can manage own series" ON public.series
    FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Public Access to obras-images" ON storage.objects
    FOR SELECT USING (bucket_id = 'obras-images');

DROP POLICY IF EXISTS "Authenticated upload to obras-images" ON storage.objects;
CREATE POLICY "Authenticated upload to obras-images" ON storage.objects
    FOR INSERT WITH CHECK (bucket_id = 'obras-images' AND auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Public Access to perfil" ON storage.objects;
CREATE POLICY "Public Access to perfil" ON storage.objects
    FOR SELECT USING (bucket_id = 'perfil');

DROP POLICY IF EXISTS "Authenticated upload to perfil" ON storage.objects;
CREATE POLICY "Authenticated upload to perfil" ON storage.objects
