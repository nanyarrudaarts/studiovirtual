...
export default function Dashboard() {
  ...
  // ⚡ BOLT OPTIMIZATION: Memoize locale to prevent redundant object selection on every render
  ...
  useEffect(() => {
    async function loadDashboardData() {
      // Avoid redundant loading state if already loading or if we wanted to prevent flash
      // but keeping it simple as per original logic.
      setLoading(true);
      try {
        // ⚡ BOLT OPTIMIZATION: Parallelize Supabase queries to eliminate network waterfall
        const [recentRes, availableRes, totalRes] = await Promise.all([
          supabase
            .from('artworks')
            .select('*')
        ...
        const obrasData = recentRes.data;
          ...
          obrasData && obrasData.length > 0
            ? Math.round(
                (obrasData.reduce((acc, o) => {
                  let filled = 0;
                  // Fixed field names to match Artwork type (performance-ready check)
        ...
        setMetricas({
          totalObras: totalRes.count || 0,
          healthScore,
          alertasMateriais: availableRes.count || 0,
        });
      } catch (err) {
  ...
  // ⚡ BOLT OPTIMIZATION: Memoize date string to avoid expensive formatting on every render
