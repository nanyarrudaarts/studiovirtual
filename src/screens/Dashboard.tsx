...
export default function Dashboard() {
  ...
  // ⚡ BOLT OPTIMIZATION: Memoize locale to avoid redundant object allocations
  ...
  useEffect(() => {
    async function loadDashboardData() {
      setLoading(true);
      try {
        // ⚡ BOLT OPTIMIZATION: Parallelize queries to eliminate waterfall bottleneck
        const [
          { data: obrasData },
          ...
          obrasData && obrasData.length > 0
            ? Math.round(
                (obrasData.reduce((acc, o) => {
                  let filled = 0;
                  // ⚡ BOLT OPTIMIZATION: Correct field names from Artwork interface
