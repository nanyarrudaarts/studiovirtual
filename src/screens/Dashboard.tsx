...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function loadDashboardData() {
      setLoading(true);
      try {
        // ⚡ BOLT OPTIMIZATION: Parallelize data fetching to eliminate waterfall
        const [
          { data: obrasData },
          ...
          obrasData && obrasData.length > 0
            ? Math.round(
                (obrasData.reduce((acc, o) => {
                  let filled = 0;
                  // ⚡ BOLT OPTIMIZATION: Correct property names according to Artwork type
  ...
  // ⚡ BOLT OPTIMIZATION: Memoize data string to prevent re-calculation on every render
  const dataAtual = useMemo(() => {
    return format(new Date(), "EEEE, d 'de' MMMM", { locale: localeObj });
  }, [localeObj]);
