...
export default function Dashboard() {
  ...
  // ⚡ BOLT OPTIMIZATION: Memoize health score calculation using correct Artwork field names
  const healthScore = useMemo(() => {
    if (obras.length === 0) return 100;
    const score = obras.reduce((acc, o) => {
      let filled = 0;
    ...
    return Math.round((score / obras.length) * 100);
  }, [obras]);

  // ⚡ BOLT OPTIMIZATION: Memoize locale object to prevent redundant object creation
  ...
  // ⚡ BOLT OPTIMIZATION: Memoize current date formatting
