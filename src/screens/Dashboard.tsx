...
export default function Dashboard() {
  ...
  // Memoize locale to prevent redundant allocations
  ...
  useEffect(() => {
    async function loadDashboardData() {
      setLoading(true);
      try {
        // ⚡ BOLT OPTIMIZATION: Parallelize queries to eliminate request waterfall
        const [recentResponse, alertasResponse, totalResponse] = await Promise.all([
          supabase
            .from('artworks')
            .select('*')
        ...
        const obrasData = recentResponse.data;
        const alertasMateriais = alertasResponse.count;
        const totalObras = totalResponse.count;

        if (obrasData) setObras(obrasData);

        // ⚡ BOLT OPTIMIZATION: Fixed property names to match Artwork interface
        const healthScore =
