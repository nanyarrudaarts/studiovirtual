...
export default function Dashboard() {
  ...
  // ⚡ BOLT OPTIMIZATION: Memoize locale to prevent redundant allocations
  ...
  useEffect(() => {
    async function loadDashboardData() {
      setLoading(true);
      try {
        // ⚡ BOLT OPTIMIZATION: Eliminate request waterfall using Promise.all
  ...
  // ⚡ BOLT OPTIMIZATION: Memoize healthScore calculation based on artworks state
  const healthScore = useMemo(() => {
    ...
    return Math.round(
      (obras.reduce((acc, o) => {
        let filled = 0;
        // Correct field names from Artwork interface
  ...
  // ⚡ BOLT OPTIMIZATION: Memoize current date string
