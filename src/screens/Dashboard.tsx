...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function loadDashboardData() {
      setLoading(true);
      try {
        // ⚡ BOLT OPTIMIZATION: Parallelizing independent Supabase queries to eliminate sequential waterfall
        const [
          { data: obrasData },
          ...
          obrasData && obrasData.length > 0
            ? Math.round(
                (obrasData.reduce((acc, o) => {
                  let filled = 0;
                  // ⚡ BOLT OPTIMIZATION: Fixed property names to match Artwork type for accurate health score
