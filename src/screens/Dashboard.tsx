...
export default function Dashboard() {
  ...
  // ⚡ BOLT OPTIMIZATION: Memoize locale to avoid object allocation on every render
  ...
  // ⚡ BOLT OPTIMIZATION: Parallelize Supabase queries to avoid request waterfall
  useEffect(() => {
    async function loadDashboardData() {
      ...
      try {
        ...
        setMetricas({
          ...
          alertasMateriais: alertasRes.count || 0,
        });
      } catch (err) {
  ...
  // ⚡ BOLT OPTIMIZATION: Derive healthScore from obras state using useMemo
  const healthScore = useMemo(() => {
    if (!obras || obras.length === 0) return 100;

    const totalScore = obras.reduce((acc, o) => {
      let filled = 0;
      // Fixed field names to match Artwork type
