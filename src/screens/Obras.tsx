...
export default function Obras() {
  ...
  const executeDelete = async () => {
    ...
    try {
      if (type === 'artwork') {
        ...
        setToast({ message: `Obra "${title}" deletada com sucesso!`, type: 'success' });
      } else if (type === 'series') {
        await deleteSerie(id);
        setSeries(prev => prev.filter(s => s.series_id !== id));
        setToast({ message: `Série "${title}" deletada com sucesso!`, type: 'success' });
      } else if (type === 'collection') {
        await deleteCollection(id);
        setCollections(prev => prev.filter(c => c.collection_id !== id));
        setToast({ message: `Coleção "${title}" deletada com sucesso!`, type: 'success' });
      }
  ...
  // ⚡ BOLT OPTIMIZATION: Memoize filtered lists to prevent redundant computations on re-renders
          ...
          <div className="relative z-10 bg-surface w-full md:max-w-4xl max-h-[92vh] md:max-h-[88vh] rounded-t-3xl md:rounded-2xl overflow-y-auto shadow-2xl">
            <div className="sticky top-0 z-20 bg-surface border-b border-gray-100 px-6 py-4 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  aria-label="Deletar"
                  onClick={() => handleDelete(selected.artwork_id, selected.artwork_title, 'artwork')}
                  className="p-2 rounded-xl hover:bg-red-100 text-red-600"
                >
                  <Trash2 size={20} />
                </button>
